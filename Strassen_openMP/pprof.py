...
def parse_cmdline(arglist):
    ...
    try:
        optlist, files = getopt.getopt(arglist, 'hl:c:x:y:t:',
                                       ["column=", "help",
                                        "legend", "linestyle=",
                                        "sep=",
                                        "log=", "term=", "title=",
    ...
    except getopt.error, e: commandline_err("%s" % str(e))

    if len(files) < 2: usage()

    for opt, arg in optlist:
        if opt in ("-c", "--column"):
            options.datacol = atoi(arg)
        elif opt in ("-h", "--help"):
            usage()
        elif opt == "--linestyle":
            options.linestl = arg
            ...
            options.maxlimit = atof(arg)
        elif opt == "--min-limit":
            options.minlimit = atof(arg)
        elif opt == "--x-limit":
            options.xlimit = atof(arg)
